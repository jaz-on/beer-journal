...
### Changed
- **Code prefix `jt_` / `JT_` / `_jt_`**: all former `jb_*` options, `_jb_*` post meta, `JB_*` PHP classes, `jt_*` global functions, `jt_*` hooks/filters, AJAX actions, admin CSS/JS `jt-` classes, and `JT_VERSION` / `JT_PLUGIN_*` constants. On load, `JT_Storage_Migration::maybe_migrate_jb_prefix_storage_to_jt()` copies legacy `jb_*` options to `jt_*`, renames meta keys, purges `jb_*` transients, and clears old cron/Action Scheduler hook registrations so the next scheduler pass uses only `jt_*` hook names. Regenerated `languages/jardin-toasts.pot` via `wp i18n make-pot`.
